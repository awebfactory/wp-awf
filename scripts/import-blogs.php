<?php
/**
 * Upsert blogs from JSON migration file exported from Drupal 6
 * 
 * execution
 * 
 * $ wp eval-file scripts/import-blogs.php data/inv-blogs.json
 */

// get file path as command-line argument argument
global $argv;
// print_r($argv, false);
// $legacy_blogs = get_array($argv[3]);

$legacy_blogs = json_decode(file_get_contents($argv[3]), true);
// print_r($legacy_blogs, false);
// print count($legacy_blogs) . "\n";
...
foreach ($legacy_blogs as $legacy_blog) {
    ...
    $the_post = array(
        ...
        'post_name'    => strval($legacy_blog['nid']),
        'post_content'  => $legacy_blog['body'],
        'post_excerpt' => $legacy_blog['teaser'],
        'post_status' => $legacy_blog['status'] == 1 ? 'publish' : 'draft',
    ...
    if ($post_id = post_exists($post_title)) {
        ...
        if ($post_id) {
            if ($legacy_blog['sticky']) stick_post( $post_id );
            echo "\nupdated: " . $post_title . "\n"; 
        } else {
